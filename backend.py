...
# get the formatted address from get request
def get_address(lat, lng):
    ...
    # key || technically this method could also be in the google_api_search class
    api_key = 'AIzaSyDzs7Yezb7ZUCCajqPY0yjRR4LBUkA2Ugw'

    # creating the full URL
    URL = URL + lng + lat + '&' + api_key
    ...
    address = data['results'][0]['formated_address']
    return address

# get the city to format the link to grab all restaurants in the city instead of hard coding the city
def get_city(lat, lng):
    ...
    # accessing the formatted address
    state = str(data['results'][0]['address_components'][-3])
    city = str(data['results'][0]['address_components'][-5])
...
# for custom searching, could be useful for an alternative method of webscraping
def format_link(address):
    URL_base = "https://www.allmenus.com/custom-results/-/"
    address = address.replace(', ', '%2C%20')
...
# expanding with other locations, assuming you are passed lat + long
def collect_restaurants(rad,lat,long):
    # running the search
    all = set(google_api_search(query='restaurant|dining', radius=rad).search_parse(lat, long))

    # split the geo_list into two separate arrays, one for purely names and one for locations
    names, locations = zip(*all)
    map_locations = dict(zip(names, locations))

    # WEB SCRAPING

    # starting link for allmenus, call BeautifulSoup
    # address_link = get_address(lat, long)
    # URL = format_link(address_link)
    city_specific = get_city(lat, long)
    URL = "https://www.allmenus.com/" + city_specific + '-/'
    ...
    for link in all_possible_links:
        if link.has_attr('href'):
            ...
            wanted_href = raw_href.replace(city_specific, "")
    ...
    URL_base2 = "https://www.allmenus.com/" + city_specific
